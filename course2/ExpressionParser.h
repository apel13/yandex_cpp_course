...
#ifndef YANDEX_CPP_COURSE_EXPRESSIONPARSER_H
#define YANDEX_CPP_COURSE_EXPRESSIONPARSER_H
...
shared_ptr <Node> Parse(Iterator token, Iterator end, const int &x) {
    ...
    auto PopOps = [&](int precedence) {
        while (!ops.empty() && ops.top()->precedence >= precedence) {
            auto value1 = values.top(); values.pop();
            auto value2 = values.top(); values.pop();
            auto op = ops.top();
            ops.pop();
...
#endif //YANDEX_CPP_COURSE_EXPRESSIONPARSER_H
